  ...
  public class HttpEncoder
  {
    ...
    private static string EncodeHeaderString(string input)
    {
      ...
      for (int index = 0; index < input.Length; ++index)
      {
        char ch = input[index];
        if (ch < ' ' && ch != '\t' || ch == '\u007F')
          StringBuilderAppend(string.Format("%{0:x2}", (object) (int) ch), ref sb);
      }
    ...
    protected internal virtual byte[] UrlEncode(byte[] bytes, int offset, int count) => UrlEncodeToBytes(bytes, offset, count);
