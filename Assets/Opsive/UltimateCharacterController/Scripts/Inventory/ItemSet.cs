...
namespace Opsive.UltimateCharacterController.Inventory {
    ...
    public class ItemSet : StateObject {
        [Tooltip("The Item Definitions that occupy the inventory slots.")] [SerializeField]
        protected ItemDefinitionBase[] m_Slots;

        [Tooltip("The state to change to when the ItemSet is active.")] [SerializeField]
        protected string m_State;

        [Tooltip("Is the ItemSet enabled?")] [SerializeField]
        protected bool m_Enabled = true;

        [Tooltip("Can the ItemSet be switched to by the EquipNext/EquipPrevious abilities?")] [SerializeField]
        protected bool m_CanSwitchTo = true;

        [Tooltip("The ItemSet index that should be activated when the current ItemSet is active and disabled.")] [SerializeField]
        protected int m_DisabledIndex = -1;
        ...
        public ItemSet(ItemSet itemSet) {
            ...
            Array.Copy(itemSet.Slots, m_Slots, itemSet.Slots.Length);
            ...
            if (itemSet.ItemIdentifiers != null && itemSet.ItemIdentifiers.Length > 0) Array.Copy(itemSet.ItemIdentifiers, m_ItemIdentifiers, itemSet.ItemIdentifiers.Length);
            if (itemSet.States != null && itemSet.States.Length > 1) {
                m_States = new State[itemSet.States.Length];
                Array.Copy(itemSet.States, m_States, itemSet.States.Length);
            }
        }
        ...
        public ItemDefinitionBase[] Slots {
            get => m_Slots;
            set => m_Slots = value;
        }
        ...
        public bool CanSwitchTo {
            get => m_CanSwitchTo;
            set => m_CanSwitchTo = value;
        }

        public int DisabledIndex {
            get => m_DisabledIndex;
            set => m_DisabledIndex = value;
        }

        public IItemIdentifier[] ItemIdentifiers {
            get => m_ItemIdentifiers;
            set => m_ItemIdentifiers = value;
        }
        ...
        public void Initialize(GameObject gameObject, ItemSetManagerBase itemSetManager, uint categoryID, int categoryIndex, int index) {
            ...
            if (m_ItemSetManager != null) return;
            ...
            m_Index = index;
            if (m_ItemIdentifiers == null) m_ItemIdentifiers = new IItemIdentifier[m_Slots.Length];
            m_EmptyItemSet = true;
        ...
        public override void StateChange() {
            ...
            if (m_Active) {
                if (m_Enabled) {
                    ...
                    if ((targetItemSetIndex == -1 || targetItemSetIndex == m_ItemSetManager.GetDefaultItemSetIndex(m_CategoryIndex)) && targetItemSetIndex != m_Index) m_EquipUnequip.StartEquipUnequip(m_Index);
                }
                else {
                ...
                var targetItemSetIndex = m_EquipUnequip.IsActive ? m_EquipUnequip.ActiveItemSetIndex : m_ItemSetManager.ActiveItemSetIndex[m_CategoryIndex];
                if (targetItemSetIndex == -1) m_EquipUnequip.StartEquipUnequip(m_Index);
            }
        }
        ...
        private void OnUpdateItemSet(int categoryIndex, int itemSetIndex) {
            if (categoryIndex == m_CategoryIndex || !m_Enabled) return;

            var activeItemSetIndex = m_ItemSetManager.ActiveItemSetIndex[m_CategoryIndex];
            if (activeItemSetIndex != -1) return;

            if (m_EquipUnequip == null) return;

            // The ItemSet may need to be enabled.
            if (!m_EquipUnequip.IsActive) m_EquipUnequip.StartEquipUnequip(m_Index);
        }
    ...
    public class CategoryItemSet {
        ...
        protected uint m_CategoryID;

        [Tooltip("The name of the category.")] [SerializeField]
        protected string m_CategoryName;

        [Tooltip("The index of the ItemSet that should be activated when no other ItemSets are activated.")] [SerializeField]
        protected int m_DefaultItemSetIndex = -1;

        [Tooltip("A list of the belonging ItemSets.")] [SerializeField]
        ...
        public uint CategoryID {
            get {
                if (RandomID.IsIDEmpty(m_CategoryID)) m_CategoryID = Category.GenerateID();
                return m_CategoryID;
            }
        ...
        public int DefaultItemSetIndex {
            get => m_DefaultItemSetIndex;
            set => m_DefaultItemSetIndex = value;
        }

        public List<ItemSet> ItemSetList {
            get => m_ItemSetList;
            set => m_ItemSetList = value;
        }

        public IItemCategoryIdentifier ItemCategory {
            get => m_ItemCategory;
