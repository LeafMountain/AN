...
namespace UI
{
    public class UIPlayerInventory
    {
        ...
        public void Init(StatefulComponent view)
        {
            ...
            GameManager.ItemManager.AddCallback(localPlayer.GetComponent<PlayerInventory>().InventoryHandle,
        ...
        {
            if (view.TryGetContainer((int)ContainerRole.PlayerInventory,
                    out ContainerReference playerInventoryContainer))
            {
                playerInventoryContainer.Container.gameObject.SetActive(!playerInventoryContainer.Container.gameObject
                    .activeSelf);
            }
        }
            ...
            if (view.TryGetContainer((int)ContainerRole.PlayerInventory, out var playerInventoryContainer))
            {
                ...
                playerInventoryContainer.Container.FillWithItems(items, (childView, handle) =>
                {
                    ...
                    if (item.HasValue)
                    {
                        ...
                        itemData.graphics.InstantiateAsync(attachPoint.Object.transform.position,
                            attachPoint.Object.transform.rotation, attachPoint.Object.transform).Completed += x =>
                        {
